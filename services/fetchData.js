...
const USERNAME = YOUR_USERNAME_HERE; // Username for API call, replace with your own username as a string
...
const CITY_URL = `&featureClass=P&featureCode=PPL&featureCode=PPLA&featureCode=PPLA2&featureCode=PPLA3&featureCode=PPLA4&featureCode=PPLA5&featureCode=PPLC`; // Url part for searching for cities of different sizes using class and codes from API
...
export const fetchData = (
  ...
  if (type === 1) {
    ...
    fetch(GEO_URL)
      ...
      .then((code) => {
        GEO_URL = `${START_GEO_URL}country=${code}${CITY_URL}&maxRows=3&orderby=population&username=${USERNAME}`; // Url to find top 3 populated cities using country code
      })
      .then(() => {
